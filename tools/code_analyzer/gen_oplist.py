...
class _Options(argparse.Namespace):
    output_dir: str
    model_file_list_path: str
    allow_include_all_overloads: bool
...
def gen_supported_mobile_models(
    model_dicts: Sequence[ModelConfig], output_dir: str
) -> None:
...
"""
    ...
    for model_dict in model_dicts:
        debug_info_entries = cast(list[str] | None, model_dict.get("debug_info"))
        if debug_info_entries:
            debug_info = cast(_ModelDebugInfo, json.loads(debug_info_entries[0]))
...
def main(argv: Sequence[str]) -> None:
    ...
    if os.path.isfile(options.model_file_list_path):
        ...
        with open(options.model_file_list_path) as model_file:
            model_dict = cast(ModelConfig, yaml.safe_load(model_file))
