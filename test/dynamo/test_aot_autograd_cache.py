...
class AOTAutogradCacheTests(InductorTestCase):
    ...
    @inductor_config.patch("fx_graph_cache", True)
    @functorch_config.patch({"enable_autograd_cache": True})
    def test_cache_hit_for_lazy_backward_without_saved_symints(self):
        """
        Regression test for graphs whose backward can be lowered lazily without
        saving any symints. These should still save an autograd cache entry and
        hit on the next compile.
        """
