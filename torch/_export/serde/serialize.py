    ...
    if isinstance(s, (torch.SymInt, sympy.Symbol, int)):
        if symbolic_shapes.is_concrete_int(s):
            ...
            if s.node.hint is None:
                return SymInt.create(as_expr=SymExpr(expr_str=None, expr_ast=sym_expr))  # type: ignore[arg-type]
            else:
                return SymInt.create(
                    as_expr=SymExpr(expr_str=None, expr_ast=sym_expr, hint=SymExprHint.create(as_int=s.node.hint))  # type: ignore[arg-type]
                )
    else:
...
def serialize_sym_bool(s: Union[bool, torch.SymBool]) -> SymBool:
    if isinstance(s, (torch.SymBool, bool)):
        if symbolic_shapes.is_concrete_bool(s):
            ...
            return SymBool.create(as_expr=SymExpr(expr_str=None, expr_ast=serialize_sym_expr(s)))  # type: ignore[arg-type]
    else:
        raise SerializeError(
