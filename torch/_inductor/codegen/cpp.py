...
class CppKernel(Kernel):
    ...
    def codegen_loops_impl(self, loop_nest, code, worksharing):
        from .cpp_outer_loop_fusion import OuterLoopFusedKernel
...
if TYPE_CHECKING:
    from .cpp_outer_loop_fusion import OuterLoopFusedKernel, OuterLoopFusedSchedulerNode
    from .cpp_scheduling import CppScheduling, ReasonFusedNodes


def __getattr__(name: str) -> Any:
    if name in ("OuterLoopFusedKernel", "OuterLoopFusedSchedulerNode"):
        from .cpp_outer_loop_fusion import (
        ...
        globals().update(
            {
                "OuterLoopFusedKernel": OuterLoopFusedKernel,
                "OuterLoopFusedSchedulerNode": OuterLoopFusedSchedulerNode,
            }
        )
        return globals()[name]

    if name in ("CppScheduling", "ReasonFusedNodes"):
        from .cpp_scheduling import CppScheduling, ReasonFusedNodes

        globals().update(
            {
                "CppScheduling": CppScheduling,
                "ReasonFusedNodes": ReasonFusedNodes,
            }
        )
        return globals()[name]
